/***
 * @description Produce semi random names.
 * @constructor
 */
function Namer() {
  var counter = 0;
  ...
  var validateName = function(name) {
    if (!isValid.test(name))
      return haiku();
    // throw name + ' is not a valid javaScript variable name';
    return name;
  }
  ...
  var getNameByElement = function(element) {
    ...
    if (typeof element.classList !== 'undefined') {
      ...
      for (var i = 0; i < element.classList.length; i++) {
        var itm = element.classList[i];
        itm = itm.camelize();
        itm = validateName(itm);
        if (itm === '' || isInUse(itm))
          continue;
      ...
      if (element.classList.length !== 0) {
        ...
        do {
          classCounter++;
        } while (isInUse(validateName(element.classList[0].camelize() + classCounter)));
        var name = validateName(element.classList[0].camelize() + classCounter);
        usedNames[name] = true;
        return name;
      }
    }
  ...
  var NAMES_TYPE = {
    byElementClue: 'byElementClue',
    haiku: 'haiku'
  }
  var nameType = NAMES_TYPE.byElementClue;

  this.setNamesType = function(str) {
    if (typeof str === 'undefined' || (str !== NAMES_TYPE.byElementClue && str !== NAMES_TYPE.haiku))
      nameType = NAMES_TYPE.byElementClue;
    else
      nameType = str;
  }
  this.getName = function(element) {
    switch (nameType) {
      case NAMES_TYPE.byElementClue:
        return getNameByElement(element);
        break;
      case NAMES_TYPE.haiku:
        return haiku();
        break;
      default:
        return getNameByElement(element);
